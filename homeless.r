...
p <- plot_ly(
  ...
  text = sprintf(
    ...
    df$year,
    scales::dollar(df$funding_per_100k),
    scales::comma(df$total_homeless_per_100k)
  ),
  mode = "markers",
  showlegend = FALSE,
...
fil <- basename(URL)
if (!file.exists(fil)) download.file(URL, fil, method = "curl")
...
#' Credit for this type of visualization goes to Época magazine and Alberto Cairo.
#' Cairo, Alberto. The Functional Art: An introduction to information graphics and visualization. New Riders, 2012.

#+ plotly_national, echo = FALSE, fig.width = 12, fig.height = 8
...
plot_ly(
  ...
  layout(
    ...
    title = 'National and State Level Temporal Scatterplots of US Department of Housing & Urban Development (HUD) Unsheltered (Estimated) Homeless Population<br>contrasted with HUD Continuum of Care Program (CoC) Funding<br>',
...
#+ plotly_state, echo = FALSE, fig.width = 12, fig.height = 16
# Bonus plotly states ----
do.call(subplot,  c(plots[state_ordr$state],options)) %>%
  config(displayModeBar = F) %>%
  layout(
    autosize = T,
    width = 1152,
...
#+ national, fig.retina=2, fig.width=12, fig.height=8, include = FALSE
# national level temporal scatterplot
...
#+ state, fig.retina = 2, fig.width=12, fig.height=20, include = FALSE
